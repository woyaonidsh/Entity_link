...
def load_data(data_path, tokenizer):
    ...
    with open(data_path, 'r', encoding='utf-8') as datas:
        for data in datas:
            ...
            related.append(txt['related_entity'])
    ...
    text_token = []
    for i in tqdm(text, desc='Tokenize text: '):
        text_token.append(tokenizer.encode(i))
    left_token = []
    ...
    statistic_data(text_token, 'text')
    statistic_data(left_token, 'left_text')
    statistic_data(right_token, 'right_text')
    return entity_token, category_token, description_token, text_token, left_token, right_token
...
def batch_data(entity_token, category_token, description_token, text_token, left_token, right_token):
    ...
    return [entity_tensor, category_tensor, description_tensor, text_tensor, left_tensor, right_tensor]
